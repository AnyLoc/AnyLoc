...
class LocalArgs:
    ...
    desc_layers: List[int] = field(default_factory=lambda: [31])
...
def build_vlads(largs: LocalArgs, vpr_ds: BaseDataset, 
    ...
    dinos = [DinoV2ExtractFeatures(largs.model_type, l, 
        largs.desc_facet, device=device) for l in largs.desc_layers]
    if verbose:
    ...
    def extract_patch_descriptors(indices):
        ...
        for i in tqdm(indices, disable=(not verbose)):
            ...
            for dino in dinos:
                ...
                ret = dino(img_in)
                desc.append(ret)
            desc = torch.concat(desc, dim=0)
            desc = ein.rearrange(desc, "l n d -> n (l d)")
            desc = F.normalize(desc, dim=1)
            patch_descs.append(desc.cpu())
        patch_descs = torch.stack(patch_descs, dim=0) # [N, n_p, d_dim]
        return patch_descs
    ...
    # Get VLADs of the database # TODO: Don't remove this
    db_vlads = None
    # if verbose:
...
def main(largs: LocalArgs):
    ...
    cmap = get_cmap(largs.num_clusters)

    for j in range(largs.num_clusters):
        colors[j,:] = cmap(j)[:3]
        custom_line = Line2D([0], [0], color = cmap(j)[:3], lw=4)
