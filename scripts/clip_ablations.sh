...
n=${#clip_impls[@]}
echo "Number of CLIP implementations: $n"
n_ds=${datasets[*]}
echo "Number of datasets: $n_ds"
n_total=$(( n * n_ds ))
echo "Total number of runs: $n_total"
curr_run=0
start_time=$(date)
...
for ((i=0; i<$n; i++)); do
    ...
    for dataset in ${datasets[*]}; do
        ...
        curr_run=$((curr_run+1))
        echo "Run: $curr_run/$n_total"
        echo -ne "\e[0m"
        # Variables for experiment
        wandb_name="CLIP/${dataset}/$clip_backbone"
        wandb_group="${dataset}"
        ...
        python_cmd="$python_cmd --prog.use-wandb"
