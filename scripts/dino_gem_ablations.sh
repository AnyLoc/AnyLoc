# Dino v1 GeM Ablations
#
# Usage: bash ./scripts/dino_gem_ablations.sh
# 
# 
...
total_runs=$((
    num_datasets * num_dino_models * num_layers * num_facets
    ))
...
for dino_facet in ${dino_facets[*]}; do
    ...
    wandb_group="$dataset"
    wandb_name="DINO_GeM/l${dino_layer}_${dino_facet}/${dataset}/"
    wandb_name+="${dino_model}"
    exp_id="ablations/$wandb_name"
    python_cmd="python ./scripts/dino_gem.py"
