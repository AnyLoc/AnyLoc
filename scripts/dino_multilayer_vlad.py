# Doing VLAD with Dino descriptors from multiple layers
"""
    Basic idea is to extract descriptors from [1] and do VLAD on them.
    Descriptors from multiple layers are concatenated and then used
    for VLAD. The same facet is used for all layers.
    
    DEPRECATED: Please do not use this script.
...
class LocalArgs:
    ...
    desc_layers: List[int] = field(default_factory=lambda: [6, 11])
...
def build_vlads(largs: LocalArgs, vpr_ds: BaseDataset, 
    ...
    if vlad.can_use_cache_vlad():
        ...
        for i in tqdm(db_indices, disable=(not verbose)):
            ...
            full_db_vlad.append(descs.cpu())  # [num_descs, d_dim_new]
...
def main(largs: LocalArgs):
    ...
    results = {
        ...
        "Desc-Layer": str(largs.desc_layers),
...
largs = LocalArgs(prog=ProgArgs(vg_dataset_name="gardens"), 
        ...
        sub_sample_db_vlad=2, desc_layers=[10, 11])
print(f"Arguments: {largs}")
