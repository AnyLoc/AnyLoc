# DINO v2 global pooling (max and average) Ablations
# 
# Usage: bash ./scripts/dino_v2_gp_ablations.sh
# 
#
...
total_runs=$(($num_runs_dino * $num_facets * $num_datasets * $npms))
curr_run=0
start_time=$(date)
...
for pool_method in ${pooling_methods[*]}; do
    ...
    echo -n "=> Dataset: $dataset => Pooling: $pool_method "
    echo "---"
    curr_run=$((curr_run+1))
    ...
    wandb_name+="/l${layer}_${facet}/$dataset/${dino_model}"
    exp_id="ablations/$wandb_name"
    python_cmd="python ./scripts/dino_v2_gp.py"
