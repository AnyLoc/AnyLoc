# Dino-v2 as global descriptor
#
# Usage: bash ./scripts/dino_v2_global_ablations.sh
#
#
...
# Models
dino_models=("dinov2_vitg14")
# dino_models=("dinov2_vits14")
# Datasets
...
for dino_model in ${dino_models[*]}; do
    # Header
    echo -ne "\e[1;93m"
    echo "--- => Dataset: $dataset => DINO: $dino_model ---"
    curr_run=$((curr_run+1))
    echo "Run: $curr_run/$total_runs"
    echo -ne "\e[0m"
    # Variables for experiment
    wandb_group="${dataset}"
    wandb_name="DINO_V2_GLOBAL/${dataset}/${dino_model}"
    exp_id="ablations/${wandb_name}"
    python_cmd="python ./scripts/dino_v2_global_vpr.py"
